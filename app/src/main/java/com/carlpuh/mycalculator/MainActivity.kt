package com.carlpuh.mycalculator
...
class MainActivity : AppCompatActivity() {
    ...
    fun calcular(view: View){
        ...
        val textBoton=boton.text.toString()
        val concat=tvRes?.text.toString()+textBoton
        val sinCeros = quitarceroizq(concat)
        if(textBoton=="="){
            var resultado=0.0
            try {
            ...
            val newConcat = concat.dropLast(1)
            tvRes?.text=newConcat
        }else if(textBoton=="C"){
            tvRes?.text="0"
        }else{
            tvRes?.text=sinCeros
    ...
    fun quitarceroizq(str: String):String{
        var i=0
